...
class AtlasDataSourceInjector:
    ...
    def _gen_sql_statement(self) -> TextClause:
        ...
        return text(
            f"""
        INSERT INTO {self.atlas_db_schema_name}.source (source_id, source_name, source_key, source_connection, source_dialect) 
        SELECT nextval('{self.atlas_db_schema_name}.source_sequence'), '{config.OHDSI_WEBAPI_DATASOURCE_NAME}', '{self._gen_source_identifier()}', ' {get_omop_sql_url(scheme='jdbc:postgresql://',auth_style='parameter')}', 'postgresql';
